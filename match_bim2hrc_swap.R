library(data.table)
library(dplyr)
bimfile = commandArgs(trailingOnly=T)[1]
...
# read in data
ref = fread("/Users/pmerbaum/Desktop/HRC.r1-1.GRCh37.wgs.mac5.sites.tab.gz", header=T)
...
exclude1 = anti_join(bmatch, ref, by=c("chr","bp")) %>%
  select(id_match)
# snp action:
shared = left_join(bmatch, ref, by=c("chr","bp")) %>%
...
update = filter(shared, exclude==0) %>%
  select(id_match, ID)
# write output
fwrite(exclude, "fals_snps_exclude.txt", col.names=F, row.names=F, quote=F, sep="\t")
fwrite(flip, "fals_snps_flip.txt", col.names=F, row.names=F, quote=F, sep="\t")
fwrite(update, "fals_snps_update.txt", col.names=F, row.names=F, quote=F, sep="\t")
fwrite(swap, 'fals_snps_swap.txt',col.names=F, row.names=F, quote=F, sep="\t" )